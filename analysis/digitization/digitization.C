...
int main(int argc, char **argv)
{
	...
	pid=para("-output_name");
	if(pid>0){
	sprintf(output_name,"%s",argv[pid+1]);
	}	
	...
	double electron_per_adc_cnt=1.0;   //electron number per ADC count
	pid=para("-Electron_per_adc_cnt");
	if(pid>0){
	electron_per_adc_cnt=atof(argv[pid+1]);
	}
...
//variable for detector
	double amplification=Amplification;
	double adc_volt_range=Adc_volt_range;

	double threshold=Threshold;
//cout constant
	cout<<"sita\t"<<sita<<"\tamplification\t"<<amplification<<"\telectron_per_adc_cnt\t"<<electron_per_adc_cnt<<endl;
...
//////////////////////////////////	
	...
	if(i%10==0)cout<<i<<"---------------"<<d->m_nbHits<<endl;
...
int hits=0;
	for(int ix=1;ix<=pixel_number_x;ix++)
	{
		...
		{	
			...
			int adc_value=(a+b)/electron_per_adc_cnt;
